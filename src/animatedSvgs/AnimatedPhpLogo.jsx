...
function DrawPath({ d, delay = 0, duration = 1.4, fill, stroke, strokeWidth = 8, inView }) {
...
function FadePath({ d, fill, delay = 0, duration = 0.8, inView }) {
    ...
    return <path d={d} fill={fill} style={{ opacity }} />;
}
...
export default function AnimatedPhpLogo({ size = 128 }) {
    ...
    return (
        <svg
            ...
            width={size}
            height={Math.round(size * (94 / 112))}
            viewBox="11 16 554 292"
            style={{ display: "block" }}
            className="rotate-x-180"
        >
            <defs>
                <clipPath id="php-clip-a">
            ...
            {/* 1. Gradient ellipse */}
            <g clipPath="url(#php-clip-a)">
                <FadePath d={ELLIPSE_GRADIENT} fill="url(#php-grad)" delay={0} duration={0.7} inView={inView} />
            </g>

            {/* 2. Purple ellipse border */}
            <g clipPath="url(#php-clip-b)">
                <FadePath d={ELLIPSE_SOLID} fill="#777bb3" delay={0.3} duration={0.7} inView={inView} />
            </g>

            {/* 3. Letters — dark layer then white detail, left → middle → right */}
            <g clipPath="url(#php-clip-b)">
                <DrawPath d={LEFT_P_DARK} fill="#000" stroke="#555" strokeWidth={5} delay={0.8} duration={1.3} inView={inView} />
                <DrawPath d={LEFT_P_WHITE} fill="#fff" stroke="#aeb2d5" strokeWidth={4} delay={1.5} duration={1.2} inView={inView} />
                <DrawPath d={MID_H_DARK} fill="#000" stroke="#555" strokeWidth={5} delay={2.0} duration={1.1} inView={inView} />
                <DrawPath d={MID_H_WHITE} fill="#fff" stroke="#aeb2d5" strokeWidth={4} delay={2.6} duration={1.1} inView={inView} />
                <DrawPath d={RIGHT_P_DARK} fill="#000" stroke="#555" strokeWidth={5} delay={3.1} duration={1.3} inView={inView} />
                <DrawPath d={RIGHT_P_WHITE} fill="#fff" stroke="#aeb2d5" strokeWidth={4} delay={3.7} duration={1.2} inView={inView} />
            </g>
        </svg>
    );
}